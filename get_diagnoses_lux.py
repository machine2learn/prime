...
codes = {
    ...
    "41270": {
        ...
        "method_timing": None,
        "age": "",
        "date": "41280", # The corresponding date each diagnosis was first recorded across all their episodes in hospital
        "year": "",
        "instancing": [0],
        "array_length": 243,
        "check_timing": 1
    ...
    "41271": {
        ...
        "method_timing": None,
        "age": "",
        "date": "41281", # The corresponding date each diagnosis was first recorded in each participant's hospital inpatient records
        "year": "",
        "instancing": [0],
        "array_length": 47,
        "check_timing": 1
...
def diabetes_u_2_date_diag(instance_int, comor_field_ukbb, df_in, found_como, d_field,  diag_date_fld, cond_date_present):
    ...
    if codes[comor_field_ukbb]["date"] is not "":
        ...
        """
        debug = df_in.loc[df_in["f.eid"].eq('1199653'), 'Diabetes_2.0'][19964]
        if debug == 1.0:
            print("date diag")
            print(comor_field_ukbb)
            print(df_in.loc[df_in["f.eid"].eq('1199653'), comor_field_ukbb][19964])
            print(found_como[19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), "diab_limit_year"][19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), diag_date_fld][19964])
            exit()
        """

    return df_in

def diabetes_u_2_date_hospit(instance_int, comor_field_ukbb, df_in, found_como, d_field,  du_field, diag_date_fld, cond_date_present):

    # only if No diabetes u is established earlier df_in[du_field].ne(True)
    if codes[comor_field_ukbb]["date"] is not "":
        df_in.loc[found_como.eq(1.0) & df_in[du_field].ne(True) & ~df_in[diag_date_fld].isnull() &
                  cond_date_present[instance_int], d_field] = \
            (df_in["diab_limit_year"].le(pd.DatetimeIndex(df_in[diag_date_fld]).year))
        """
        debug = df_in.loc[df_in["f.eid"].eq('1199653'), 'Diabetes_2.0'][19964]
        if debug == 1.0:
            print("date hospitalisation")
            print(comor_field_ukbb)
            print(df_in.loc[df_in["f.eid"].eq('1199653'), comor_field_ukbb][19964])
            print(found_como[19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), "diab_limit_year"][19964])
...
def diabetes_u_2_age_diag(instance_int, comor_field_ukbb, df_in,found_como, d_field,
    ...
    if codes[comor_field_ukbb]["age"] is not "":
        ...
        if debug == 1.0:
            ...
            print(df_in.loc[df_in["f.eid"].eq('1199653'), comor_field_ukbb][19964])
            print(found_como[19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), diag_age_fld][19964])
            exit()
        """
...
def diabetes_u_2_year_diag(instance_int, comor_field_ukbb, df_in, found_como, d_field,
    ...
    if codes[comor_field_ukbb]["year"] is not "":
        ...
        if debug == 1.0:
            ...
            print(found_como[19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), "diab_limit_year"][19964])
            print(df_in.loc[df_in["f.eid"].eq('1199653'), diag_year_fld][19964])
            exit()
        """
...
def preprocess(df_in):
    ...
    for i in range(4):
        ...
        if i == 0:
            ...
            current_age = df["age_visit.0"] + (current_year - df["year_visit.0"])

        df_in.insert(1, "age_visit."+str(i), current_age)

        # making main output columns listing comorbidities
    ...
    birth_year = df["year_visit.0"] - df["age_visit.0"]
    df_in.insert(1, "birth_year", birth_year)
    ...
    for comor_col in comorbidities_fileds:
        ...
        # comor_col_split[3] is array index for multivalued fields
        ...
        if codes[comor_field_ukbb]["check_timing"] == 1: # no instancing for this ukbb field, so we have to chek time of the diagnose
            # fld 4170 and 4171
            ...
            for fld_out in out_comorb_fileds:
                ...
                diag_date = pd.to_datetime(df_in[diag_date_fld], format='%Y-%m-%d')
                date_visit = pd.to_datetime(df_in[fld_1], format='%Y-%m-%d')
                timing = ~df_in[diag_date_fld].isnull() & cond_date_present[int(i_str)] & diag_date.le(date_visit)

                df_in.loc[found_comorbidity & df_in[fld_out].ne(True), fld_out] = timing

                if fld_out == ("Diabetes_u." + i_str):
                    d_field = "Diabetes_2." + i_str
                    du_field = "Diabetes_u." + i_str
                    found_comorbidity_du = found_comorbidity & timing
...
if __name__ == '__main__':
    ...
    out_file = "/diagnoses_2023_diabetes_30.csv"
    ...
    # cognitive fields
     ...
     # n.m., max digits remembered correctly
    current_list_1 = [col for col in data_columns if col.startswith("f.4282")]
    all_fields_ = all_fields_ + current_list_1

     # s.g. mean r.t.
    current_list_2 = [col for col in data_columns if col.startswith("f.20023")]
    all_fields_ = all_fields_ + current_list_2

        # p.m. sum incorrect
    current_list_3 = [col for col in data_columns if col.startswith("f.399")]
    all_fields_ = all_fields_ + current_list_3
    ...
    for i in range(4):
        ...
        df.loc[df[date_fld].notna() & df[fld_du].eq(False) & df[fld_d2].isnull(), fld_d2] = False
    ...
    count_d2_0 = df['Diabetes_2.0'].sum()
    print(f"Ppl diagnosed diabetes 2 in visit 0: {count_d2_0}")

    count_d2_nm_0 = (df['Diabetes_2.0'] & df['f.4282.0.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in n.m. in visit 0: {count_d2_nm_0}")
    count_d2_sg_0 = (df['Diabetes_2.0'] & df['f.399.0.1'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in s.g. in visit 0: {count_d2_sg_0}")
    count_d2_pm_0 = (df['Diabetes_2.0'] & df['f.20023.0.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in p.m. in visit 0: {count_d2_pm_0}")

    count_d2_nm_2 = (df['Diabetes_2.0'] & df['f.4282.2.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in n.m. in visit 2: {count_d2_nm_2}")
    count_d2_sg_2 = (df['Diabetes_2.0'] & df['f.399.2.1'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in s.g. in visit 2: {count_d2_sg_2}")
    count_d2_pm_2 = (df['Diabetes_2.0'] & df['f.20023.2.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in p.m. in visit 2: {count_d2_pm_2}")

    count_d2_nm_02 = (df['Diabetes_2.0'] & df['f.4282.0.0'].astype(float).ge(0)
                     & df['f.4282.2.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in n.m. in visits 0,2: {count_d2_nm_02}")

    count_d2_sg_02 = (df['Diabetes_2.0'] & df['f.399.0.1'].astype(float).ge(0) &
                      df['f.399.2.1'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in s.g. in visit 0, 2: {count_d2_sg_02}")

    count_d2_pm_02 = (df['Diabetes_2.0'] & df['f.20023.0.0'].astype(float).ge(0) &
                      df['f.20023.2.0'].astype(float).ge(0)).sum()
    print(f"Ppl diagnosed diabetes 2 and took part in p.m. in visit 0, 2: {count_d2_pm_02}")
    ...
    # debug info
    set_main_2 = set(df[df['Diabetes_2.0'].eq(True)]['f.eid'])
    # print(set_main)
    set_check_2 = set(diag_20002[diag_20002['diab_2.0'].eq(True)]['f.eid'])
    # print(set_check)
    ...
    # debug info
    set_main = set(df[df['Diabetes_u.0'].eq(True)]['f.eid'])
    # print(set_main)
    set_check =set(diag_20002[diag_20002['diab_u.0'].eq(True)]['f.eid'])
    # print(set_check)
    ...
    """debug_df = df[df["Dyslipidemia.0"]]
    for row_index, row in debug_df.iterrows():
        print(row["f.eid"])
        print("****")
"""
    # df_out = df[out_fields]
    df_out = df[out_cols]
